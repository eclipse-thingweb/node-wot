...
function showProperties(thing) {
		...
		if (td.properties.hasOwnProperty(property)) {
			thing.readProperty(property).then(value => {
				...
				if (!el) {
					...
					tr.innerHTML = `<td><pre>${property}</pre></td><td id="${property}" class="property-value"><pre>${JSON.stringify(value, null, 2)}</pre></td>`;
	...
	setTimeout(function() {showProperties(thing)}, UPDATE_PERIOD);
}
